...
void Gen::InitExternalFunction() {
	// scanf
	std::vector<llvm::Type*> Args{ llvm::Type::getInt8PtrTy(*g_llvm_context) };
	FunctionType* scanfTy = FunctionType::get(GetType(Int), Args, true);
	Function* scanf = Function::Create(scanfTy, Function::ExternalLinkage, "scanf", *g_module);

	// printf
	FunctionType* printfTY = FunctionType::get(GetType(Int),Args, true);
	...
	if (g_values.find(*var->varName) == g_values.end()) {
		if (!var->is_array) {
			GlobalVariable* gVar = new GlobalVariable(*g_module,
				GetType(var->bType), false,  // ֻ�ڵ�ǰģ���ڿɼ�
				GlobalValue::ExternalLinkage,
				ConstantInt::get(*g_llvm_context, APInt(32, 0)),  // ��ʼֵΪnullptr
				*(var->varName)  // ȫ�ֱ���������
			);
			if (var->initValList != nullptr) {
				gVar->setInitializer(dyn_cast<Constant>(ValueGen(var->initValList->valList[0])));//varList������,����Ԫ�ص�������Exp*
			}
			...
			//����Ĭ��ȫ�����鲻����int a[][]={}�ĸ�ֵ
			Value* arraySize = GetArraySize(var);//lenҲ������,ÿ��������Exp*
...
void Gen::FuncGen(AST::Unit*& unit) {
	...
	if (fun->argList) {
		function_type = FunctionType::get(GetType(fun->funcType), arg_types, false);
	}
	else  function_type = FunctionType::get(GetType(fun->funcType), false);
	...
	//���ű�Ҫ���뺯�����β�
	auto* cur_block = g_ir_builder->GetInsertBlock();//cur��block��ָ��
	int i = 0;
	if (fun->argList) {
		for (auto& arg : func->args()) {
			Value* val = &arg;
			...
			NamedValues.insert(cur_arg->argName, Alloca, nullptr);//ע�������β���ָ��Ļ���д�����ű��ﲢû��ָ��ά��,��tcn��һ��
...
Value* Gen::ValueGen(AST::Exp* ast) {
	if (ast->expType == constNumExp) {
		...
		if (var == nullptr) {//var==nullptr��ԭ����ȫ�ֱ�������ʱû��AllocaInst*��֤�����Ǹ�ȫ�ֱ���
...
Value* Gen::GetArrayIndex(const AST::Variable* VarUnit) {
	Value* index = ValueGen(VarUnit->arr->len[(VarUnit->arr->len).size() - 1]);//�������ҵ�inde
	vector<AST::Exp*>* arrayLen = NamedValues.find_dim(VarUnit->varName);//�����ܲ鵽ԭ�����size
...
Value* Gen::FuncCallGen(AST::FuncCall* ast) {
	...
	if (ast->realArgList) {
		//ע�⴫ʵ�ε�ʱ���������ͻ᳤�ɵ�����������,lexp�ᱻ����ɵ����������Բ�����ô��ÿһ������ֱ��ValueGen
		for (auto& arg : ast->realArgList->realArgs) {
			if (arg->expType == lValExp) {
				...
				if (var->getAllocatedType()->isPointerTy()) {//����Ĵ�����Ϊ��funccall��ʵ������㺯���βΣ������β�ָ�������£�quicksort�����quicksort
					args.push_back(g_ir_builder->CreateLoad(var->getAllocatedType(), var, var->getName().data()));
				}
				else if (dim && !ast->arr) {
					args.push_back(g_ir_builder->CreateInBoundsGEP(var->getAllocatedType(), var, ConstantInt::get(*g_llvm_context, APInt(32, 0))));//��Ӧmain�������quicksort���
				}
